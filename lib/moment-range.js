...
/**
 * Extend date range to include given moment.
 *
 * @param {!Moment} other A moment to extend the DateRange with
 *
 * @return {DateRange} Returns the new `DateRange`
 */
DateRange.prototype.extend = function(other) {
  if(other < this.start) {
    return new DateRange(other, this.end);
  } else if(other > this.end) {
    return new DateRange(this.start, other);
  } else {
    return this.clone();
  }
};
