...
DateRange.prototype.setLimit = function (options) {
  var parsedLimit = this.parseRange(options.lowerLimit, options.upperLimit);
...
DateRange.prototype._intersect = function() {
  ...
  if ((actualStart <= lowerLimit) && (lowerLimit < actualEnd) && (actualEnd < upperLimit)) {
    this.start = lowerLimit;
    this.end = actualEnd;
  }
  else if ((lowerLimit < actualStart) && (actualStart < upperLimit) && (upperLimit <= actualEnd)) {
    this.start = actualStart;
    this.end = upperLimit;
  }
  else if ((lowerLimit < actualStart) && (actualStart <= actualEnd) && (actualEnd < upperLimit)) {
    this.start = actualStart;
    this.end = actualEnd;
  }
  else if ((actualStart <= lowerLimit) && (lowerLimit <= upperLimit) && (upperLimit <= actualEnd)) {
    this.start = lowerLimit;
    this.end = upperLimit;
  }
  else {
    throw('Cannot set range outside of range limits');
  }
};
