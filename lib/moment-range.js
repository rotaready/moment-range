...
export class DateRange {
  ...
  by(interval: Shorthand, options?: { exclusive: bool; step: number; } = { exclusive: false, step: 1 }): Iterator<moment> {
    const range = this;
  ...
  byRange(interval: DateRange, options?: { exclusive: bool; step: number; } = { exclusive: false, step: 1 }): Iterator<moment> {
    const range = this;
    const step = options.step || 1;
  ...
  reverseByRange(interval: DateRange, options?: { exclusive: bool; step: number; } = { exclusive: false, step: 1 }): Iterator<moment> {
    const range = this;
    const step = options.step || 1;
    const diff = this.valueOf() / interval.valueOf() / step;
    const exclusive = options.exclusive || false;
