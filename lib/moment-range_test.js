...
describe('Moment', function() {
  ...
  describe('#within()', function() {
    ...
    it('should consider the edges to be within the range', function() {
      dr = moment.range(m1, m2);
      expect(m1.within(dr)).to.be(true); // this test is flickering
      expect(m2.within(dr)).to.be(true);
    });
  });
});
...
describe('DateRange', function() {
  ...
  const m1 = moment.utc('05-06-1996', 'DD-MM-YYYY');
  const m2 = moment.utc('05-11-1996', 'DD-MM-YYYY');
  const m3 = moment.utc('12-08-1996', 'DD-MM-YYYY');
  const m4 = moment.utc('01-01-2012', 'DD-MM-YYYY');
  ...
  describe('#adjacent', function() {
    ...
    it('should correctly indicate when a.start == b.start', function() {
      const a = moment('15-03-2016', 'DD-MM-YYYY');
      const b = moment('29-03-2016', 'DD-MM-YYYY');
      const c = moment('15-03-2016', 'DD-MM-YYYY');
      const d = moment('30-03-2016', 'DD-MM-YYYY');
    ...
    it('should correctly indicate when a.start == b.end', function() {
      const a = moment('15-03-2016', 'DD-MM-YYYY');
      const b = moment('29-03-2016', 'DD-MM-YYYY');
      const c = moment('10-03-2016', 'DD-MM-YYYY');
      const d = moment('15-03-2016', 'DD-MM-YYYY');
    ...
    it('should correctly indicate when a.end == b.start', function() {
      const a = moment('15-03-2016', 'DD-MM-YYYY');
      const b = moment('20-03-2016', 'DD-MM-YYYY');
      const c = moment('20-03-2016', 'DD-MM-YYYY');
      const d = moment('25-03-2016', 'DD-MM-YYYY');
    ...
    it('should correctly indicate when a.end == b.end', function() {
      ...
      const b = moment('20-03-2016', 'DD-MM-YYYY');
      const c = moment('10-03-2016', 'DD-MM-YYYY');
      const d = moment('20-03-2016', 'DD-MM-YYYY');
  ...
  describe('#by', function() {
    ...
    it('should be exlusive when using by with minutes as well', function() {
      ...
      expect(acc).to.eql(['01', '02', '03', '04', '05']);
    });
    ...
    it('should correctly iterate by a given step when exclusive', function() {
      ...
      const acc = Array.from(dr1.by('days', { inclusivity: '()', step: 2 })).map(m => m.utc().format('DD'));
      expect(acc).to.eql(['04', '06']);
    });
  });
  ...
  describe('#reverseBy', function() {
    ...
    it('should be exlusive when using by with minutes as well', function() {
      ...
      expect(acc).to.eql(['05', '04', '03', '02', '01']);
    });
    ...
    it('should correctly iterate by a given step when exclusive', function() {
      ...
      const acc = Array.from(dr1.reverseBy('days', { inclusivity: '()', step: 2 })).map(m => m.utc().format('DD'));
  ...
  describe('#byRange', function() {
    ...
    it('should not include .end in the iteration if exclusive is set to true when iterating by range', function() {
      ...
      acc = Array.from(dr1.byRange(dr2, { inclusivity: '[]' })).map(m => m.utc().format('YYYY-MM-DD'));
      expect(acc).to.eql(['2014-04-02', '2014-04-03', '2014-04-04']);
    ...
    it('should iterate correctly by a given step when exclusive', function() {
      ...
      const acc = Array.from(dr1.byRange(dr2, { inclusivity: '()', step: 2 })).map(m => m.utc().format('DD'));

      expect(acc).to.eql(['04']);
    });
  });
  ...
  describe('#reverseByRange', function() {
    ...
    it('should not include .start in the iteration if exclusive is set to true when iterating by range', function() {
      ...
      acc = Array.from(dr1.reverseByRange(dr2, { inclusivity: '[]' })).map(m => m.utc().format('YYYY-MM-DD'));
      expect(acc).to.eql(['2014-04-04', '2014-04-03', '2014-04-02']);
    ...
    it('should iterate correctly by a given step when exclusive', function() {
      ...
      const acc = Array.from(dr1.reverseByRange(dr2, { inclusivity: '()', step: 2 })).map(m => m.utc().format('DD'));
  ...
  describe('#contains()', function() {
    ...
    it('should be exlusive when the exclusive param is set', function() {
      ...
      expect(dr1.contains(dr1, { inclusivity: '()' })).to.be(false);
      expect(dr1.contains(dr1, { inclusivity: '[]' })).to.be(true);
      expect(dr1.contains(dr1)).to.be(true);
      expect(dr1.contains(m2, { inclusivity: '()' })).to.be(false);
      expect(dr1.contains(m2, { inclusivity: '[]' })).to.be(true);
  ...
  describe('#overlaps()', function() {
    it('should work with DateRange objects', function() {
      ...
      expect(dr1.overlaps(dr3, { inclusivity: '()' })).to.be(false);
      expect(dr1.overlaps(dr3, { inclusivity: '[]' })).to.be(true);
    ...
    it('should indicate if ranges overlap if the options is passed in', function() {
      ...
      expect(range1.overlaps(range2, { inclusivity: '()' })).to.be(false);
      expect(range1.overlaps(range2, { inclusivity: '[]' })).to.be(true);
    });
  });
  ...
  describe('#intersect()', function() {
    ...
    it('should work with [---]{---} overlaps where (a=[], b={})', function() {
      ...
      expect(dr1.intersect(dr2, { inclusivity: '()' })).to.be(null);
      expect(dr1.intersect(dr2, { inclusivity: '[]' }).isSame(moment.range(d6, d6))).to.be(true);
    });
  ...
  describe('#add()', function() {
    ...
    it('should not add ranges with [---]{---} overlaps where (a=[], b={})', function() {
      ...
      expect(dr1.add(dr2).isSame(moment.range(d5, d7))).to.be(true);
      expect(dr1.add(dr2, { inclusivity: '()' })).to.be(null);
      expect(dr1.add(dr2, { inclusivity: '[]' }).isSame(moment.range(d5, d7))).to.be(true);
    });
